...
NS_OBJECT_ENSURE_REGISTERED (RttHeader);
...
TypeId
RttHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

void RttHeader::Print (std::ostream &os) const
{
  return;
}
uint32_t RttHeader::GetSerializedSize (void)  const
{
  return sizeof(rate)+sizeof(ih)+sizeof(dport);
}
void RttHeader::Serialize (Buffer::Iterator start)  const
{ 
...
uint32_t RttHeader::Deserialize (Buffer::Iterator start)
{

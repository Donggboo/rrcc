...
void SwitchNode::SendToDev(Ptr<Packet>p, CustomHeader &ch){
	...
	if (idx >= 0){
		...
		uint32_t qIndex;
		if (ch.l3Prot == 0xFF || ch.l3Prot == 0xFE || (/*m_ackHighPrio && */(ch.l3Prot == 0xFD || ch.l3Prot == 0xFC||ch.l3Prot == 0xFC))){  //QCN or PFC or NACK, go highest priority
			qIndex = 0;
		}else{
...
void SwitchNode::sendq(Ptr<Packet> p,uint32_t ifIndex){
	...
	RttHeader rh;
	rh.rate=ch.udp.ih.rate;
	rh.dport=ch.udp.sport;
	rh.ih.Set(Simulator::Now().GetTimeStep(), m_txBytes[ifIndex], dev->GetQueue()->GetNBytesTotal(), dev->GetDataRate().GetBitRate());
	newp->AddHeader(rh);
	Ipv4Header ipv4h;  // Prepare IPv4 header
	ipv4h.SetProtocol(0xFB);
	uint32_t id= 0x0b000001 + ((m_id / 256) * 0x00010000) + ((m_id % 256) * 0x00000100);
	ipv4h.SetSource(Ipv4Address(id));

...
uint64_t RdmaHw::GetQpKey(uint32_t sip, uint16_t sport, uint16_t pg){
	return ((uint64_t)sip << 32) | ((uint64_t)sport << 16) | (uint64_t)pg;
}
Ptr<RdmaQueuePair> RdmaHw::GetQp(uint32_t sip, uint16_t sport, uint16_t pg){
	uint64_t key = GetQpKey(sip, sport, pg);
...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish){
	...
	if(it==m_qpMap.end()){
		...
		printf("%d %d %d %d \n",it->second->sip.Get(),sip.Get(),it->second->sport,sport);
		NS_ASSERT(false);
	}
...
void RdmaHw::DeleteQueuePair(Ptr<RdmaQueuePair> qp){
	...
	NS_ASSERT(qp->IsFinished());
	uint64_t key = GetQpKey(qp->sip.Get(), qp->sport, qp->m_pg);
	m_qpMap.erase(key);
}
...
int RdmaHw::ReceiveRtt(Ptr<Packet> p, CustomHeader &ch){
		uint16_t port = ch.rtt.dport;
...
void RdmaHw::QpComplete(Ptr<RdmaQueuePair> qp){
    ...
    NS_ASSERT(qp->snd_una>=qp->m_size);
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp){
	...
	//std::cout<<qp->snd_nxt<<std::endl;
	seqTs.SetPG (qp->m_pg);
	IntHeader ih;
	if(m_cc_mode==11)
		ih.rate=qp->m_rate/DataRate(50000000);
	seqTs.ih=ih;
	p->AddHeader (seqTs);
	// add udp header
	UdpHeader udpHeader;
...
void RdmaHw::UpdateRateHp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react){
	...
	if (qp->hp.m_lastUpdateSeq == 0){ // first RTT
		...
		// check packet INT
		IntHeader &ih = ch.ack.ih;
		//printf("%d %d\n",ih.nhop,Simulator::Now().GetTimeStep());
			...
			if (!m_multipleRate){
				...
				if (updated_any){
					...
					if (max_c >= 1 || qp->hp.m_incStage >= m_miThresh){
						...
						if(max_c<1){
							new_rate = qp->hp.m_curRate / max_c;
						}else{
...
void RdmaHw::HandleAckHpPint(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){
	...
	void RdmaHw::HandleRTT(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch){

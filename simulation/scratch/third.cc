...
void getINFB(FILE* f, NodeContainer *n){
	if(Simulator::Now().GetTimeStep() % 200 == 0&&Simulator::Now().GetTimeStep()<500000){
		uint64_t bit=0;
		for (uint32_t i = 0; i < n->GetN(); i++){
			if(n->Get(i)->GetNodeType()==0){
				Ptr<Node> node=n->Get(i);
				Ptr<QbbNetDevice> dev=DynamicCast<QbbNetDevice>(node->GetDevice(1));
				for(int k=0;k<dev->m_rdmaEQ->m_qpGrp->GetN();k++){
					Ptr<RdmaQueuePair> qp=dev->m_rdmaEQ->GetQp(k);
					bit+=qp->snd_nxt;
					bit-=qp->snd_una;
				}
		...
		fprintf(f,"%ld %ld\n",bit,Simulator::Now().GetTimeStep());
	}
	
	Simulator::Schedule(NanoSeconds(200), &getINFB,f,n);
}
...
int main(int argc, char *argv[])
{
    ...
    FILE *inflightbit=  fopen("inflightbit.txt","w");
	//Simulator::Schedule(NanoSeconds(0), &getINFB, inflightbit, &n);
	//
